		...
		<div class="combobox__wrap">
			<div class="combobox__form">
				<input
					...
					readonly
				    v-model="locationInfo.currentLocation"
				/>
				<i class="combobox__icon"></i>
			</div>
			<ul class="combobox__list">
				<li v-for="item in locationInfo.locationList" :key="item.id">
...
const locationInfo = reactive({
	locationList : null,
	currentLocation : computed(() => locationInfo.currentLocation ? locationInfo.currentLocation : '지역선택')
});

async function fetchData() {
	locationInfo.locationList = await store.getLocations();
}
onMounted(() => {
	fetchData();
});
